    ...
    for (int i = 0; !decoderResult && i < 4; i++) {
      ...
      for (FormatReader *reader in formatReaders) {
        ...
        try {
          ...
          const char *cFormat = BarcodeFormat::barcodeFormatNames[barcodeFormat];
          ...
          resultFormat = [[NSString alloc] initWithCString:cFormat encoding:NSUTF8StringEncoding];
            
          if (decoderResult) [decoderResult release];
          decoderResult = [[TwoDDecoderResult alloc] initWithText:resultString points:points format: resultFormat];
